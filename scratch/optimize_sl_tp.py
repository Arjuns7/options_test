...
def run_backtest_params(sl_pct, tp_pct):
    active_trade = None
    trades = []
...
sl_values = [0.15, 0.20, 0.25, 0.30]
tp_values = [0.20, 0.25, 0.30, 0.35, 0.40]
...
for sl in sl_values:
    for tp in tp_values:
        trades = run_backtest_params(sl, tp)
...
grid_results = sorted(grid_results, key=lambda x: x['pnl'], reverse=True)

print("\n=======================================================")
print("   GRID SEARCH OPTIMIZATION: SL vs. TARGET (6-MONTH)   ")
print("   Nifty 15-Min EMA Crossover Strategy + ADX > 25      ")
print("=======================================================")
print(f"{'Rank':<5} | {'SL (%)':<8} | {'Target (%)':<10} | {'Trades':<8} | {'Win Rate':<10} | {'Net P&L (INR)':<15}")
print(f"{'-'*5} | {'-'*8} | {'-'*10} | {'-'*8} | {'-'*10} | {'-'*15}")

for rank, r in enumerate(grid_results[:12], 1):
    pnl_str = f"{'+' if r['pnl'] >= 0 else ''}INR {r['pnl']:,.2f}"
    print(f"{rank:<5} | {r['sl']:<8} | {r['tp']:<10} | {r['trades']:<8} | {r['win_rate']:<9}% | {pnl_str:<15}")
