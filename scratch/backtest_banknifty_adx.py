...
def get_expiry_date(date_str):
    ...
    # Bank Nifty weekly expiry was historically Thursday or Wednesday
    # Let's assume standard Thursday expiry (weekday = 3)
...
df = pd.DataFrame(data["BANKNIFTY"])
...
def run_backtest_banknifty(filter_mode='NONE'):
    active_trade = None
    trades = []
    ...
    for i in range(2, len(df)):
        ...
        # 2. Check for Crossover Signals
            ...
            if signal != 'NONE' and tick_idx < 24:
                ...
                if filter_mode == 'ADX_STRENGTH_25':
                    if adx <= 25:
                        continue
                
                option_type = 'C' if signal == 'BULLISH' else 'P'
                # BANK NIFTY rounded to nearest 100!
...
results = {}
for key in ['NONE', 'ADX_STRENGTH_25']:
    trades = run_backtest_banknifty(key)
...
print("\n==========================================")
print("   6-MONTH BANK NIFTY BACKTEST REPORT     ")
print("   (Bank Nifty 15-Min EMA Crossover)      ")
print("==========================================")
print("Dataset Range: January 14, 2026 to July 13, 2026")
print("Trade Size: 30 units (2 Lots)\n")

print(f"1. Baseline (No Filter):")
res = results['NONE']
...
print(f"\n2. With ADX > 25 Filter:")
res = results['ADX_STRENGTH_25']
print(f"  * Total Trades: {res['count']}")
print(f"  * Wins: {res['wins']} | Losses: {res['losses']}")
print(f"  * Win Rate: {res['win_rate']}%")
print(f"  * Net P&L: {'+' if res['net_pnl'] >= 0 else ''}INR {res['net_pnl']:,}")
...
print("               ANALYSIS                   ")
print("==========================================")
pnl_diff = results['ADX_STRENGTH_25']['net_pnl'] - results['NONE']['net_pnl']
trades_avoided = results['NONE']['count'] - results['ADX_STRENGTH_25']['count']
print(f"* ADX > 25 blocked {trades_avoided} low-trend trades.")
